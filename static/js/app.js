...
class SafeCart {
    ...
    renderCartItems() {
        ...
        cartItemsContainer.innerHTML = this.cart.map(item => `
            <div class="cart-item">
                ...
                <div class="cart-item-price">${currencyReady && typeof currencyManager !== 'undefined' ? currencyManager.formatPrice(item.price * item.quantity) : '$' + (item.price * item.quantity).toFixed(2)}</div>
                <button class="btn btn-remove" onclick="safeCart.removeFromCart('${item.id}')">Remove</button>
            </div>
        `).join('');
        ...
        wishlistItemsContainer.innerHTML = this.wishlist.map(item => `
            <div class="wishlist-item">
                ...
                <div class="wishlist-item-details">
                    <h4>${item.name}</h4>
                    <p>${currencyReady && typeof currencyManager !== 'undefined' ? currencyManager.formatPrice(item.price) : '$' + item.price.toFixed(2)}</p>
                    <div class="wishlist-item-actions">
                        <button class="btn btn-cart" onclick="safeCart.moveToCart('${item.id}')">Add to Cart</button>
                        <button class="btn btn-remove" onclick="safeCart.removeFromWishlist('${item.id}')">Remove</button>
                    </div>
                </div>
            </div>
        `).join('');
    ...
    updateCartSummary() {
        ...
        if (currencyReady && typeof currencyManager !== 'undefined') {
            if (subtotalElement) subtotalElement.textContent = currencyManager.formatPrice(subtotal);
            if (taxElement) taxElement.textContent = currencyManager.formatPrice(tax);
            if (totalElement) totalElement.textContent = currencyManager.formatPrice(total);
        } else {
    ...
    checkout() {
        ...
        const formattedTotal = currencyReady && typeof currencyManager !== 'undefined' 
            ? currencyManager.formatPrice(total) 
            : `$${total.toFixed(2)}`;
        this.showNotification(`Checkout successful! Total: ${formattedTotal}`, 'success');
...
if (paymentMethodForm) {
    paymentMethodForm.addEventListener('submit', function(event) {
        ...
        if (method === 'upi') {
            ...
            setTimeout(function() {
                document.getElementById('payment-complete-btn').style.display = 'inline-block';
            }, 3000);
...
if (codMobileForm) {
    codMobileForm.addEventListener('submit', function(event) {
        ...
        if (!/^\d{10}$/.test(mobile)) {
            codOtpMessage.textContent = 'Please enter a valid 10-digit mobile number.';
            return;
        }
...
if (codOtpForm) {
    codOtpForm.addEventListener('submit', function(event) {
        ...
        if (enteredOtp === generatedOtp) {
            ...
            document.getElementById('qr-step').innerHTML = '<h3>Thank you for your purchase!</h3><p>Your order has been confirmed and will be shipped soon.</p>';
            document.getElementById('qr-step').style.display = 'block';
...
if (paymentBtn) {
    paymentBtn.addEventListener('click', function() {
        document.getElementById('qr-step').innerHTML = '<h3>Thank you for your purchase!</h3><p>Your payment has been received. Your order will be shipped soon.</p>';
        if (safeCart.cart && safeCart.cart.length > 0) {
            const orders = JSON.parse(localStorage.getItem('orders') || '[]');
            const now = new Date();
            const maxDays = 5;
...
window.addEventListener('offline', () => {
    safeCart.showNotification('You are offline. Some features may not work.', 'info');
});
...
function updateProfileDropdown() {
    ...
    if (!profileBtn || !profileDropdown || !profileContent || !profileMenu) return;
    const user = JSON.parse(localStorage.getItem('user') || 'null');
    if (user && user.email) {
